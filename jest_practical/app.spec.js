...
test("app.js가 실행될 때, sequelize.sync가 성공하면 then을 호출한다.", () => {
    jest.spyOn(sequelize, "sync").mockResolvedValue(undefined);
    const app = require("./app");
    expect(app).toBeDefined();
});

test("app.js가 실행될 때, sequelize.sync가 실패하면 catch를 호출한다.", () => {
    jest.spyOn(sequelize, "sync").mockRejectedValue(new Error());
    const app = require("./app");
    expect(app).toBeDefined();
});

test("notFoundHandler가 잘 실행된다.", () => {
...
test("NODE_ENV가 production이 아닐 때 errorHandler가 error와 함께 실행된다.", () => {
...
test("NODE_ENV가 production일 때 errorHandler가 error 없이 실행된다.", () => {
