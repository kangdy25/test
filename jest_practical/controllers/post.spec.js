...
test("afterUploadImage는 res.json으로 url을 반환해야 한다.", () => {
    const req = {
        file: {
            filename: "test.png",
        },
...
describe("uploadPost", () => {
    test("게시글 등록 시 실패 시, 에러처리 함수를 호출한다.", async () => {
    ...
    test("게시글 등록 성공 시, 해시태그가 없다면 /로 돌려보낸다.", async () => {
        const newPost = {
            addHashtags: jest.fn(),
        };
    ...
    test("게시글 등록 성공 시, 해시태그가 있으면 해시태그까지 저장 후 /로 돌려보낸다.", async () => {
        const newPost = {
            addHashtags: jest.fn(),
        };
        ...
        expect(Hashtag.findOrCreate.mock.calls[1][0].where.title).toBe(
            "해시태그"
        );
        expect(Hashtag.findOrCreate.mock.calls[2][0].where.title).toBe(
            "dongyoon"
        );
        expect(newPost.addHashtags).toHaveBeenCalledTimes(1);
        expect(newPost.addHashtags).toHaveBeenCalledWith([
            "결과1",
            "결과2",
            "결과3",
        ]);
