...
test("패스포트 초기화 코드에서 serializeUser, deserializeUser, kakao, local 함수가 잘 실행된다", () => {
...
test("serializeUser 유저 아이디를 done 콜백으로 호출해야한다", () => {
    const done = jest.fn();
...
test("deserializeUser 유저 객체를 done 콜백으로 호출해야한다", () => {
    const done = jest.fn();
...
test("deserializeUser 유저 조회 시 에러가 나면 done 콜백으로 에러를 호출해야한다", () => {
    const done = jest.fn();
    const { afterDeserialize } = require("./index");
