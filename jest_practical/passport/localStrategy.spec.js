...
describe("localStrategy", () => {
    test("유저가 존재하고 비밀번호도 일치하면 done 콜백을 호출한다", async () => {
        const done = jest.fn();
        jest.spyOn(User, "findOne").mockResolvedValue({ id: 1 });
        jest.spyOn(bcrypt, "compare").mockResolvedValue(true);

        await localStrategyInit.localStrategy(
            "zerohch0@gmail.com",
            "password",
            done
        );
    ...
    test("유저가 존재하고 비밀번호가 틀리면 메시지와 함께 done 콜백을 호출한다", async () => {
        const done = jest.fn();
        jest.spyOn(User, "findOne").mockResolvedValue({ id: 1 });
    ...
    test("유저가 존재하지 않는 경우 메시지와 함께 done 콜백을 호출한다", async () => {
        const done = jest.fn();
